module kizuna::battle {
    ...
    public struct BattleAdminCap has key {
        id: sui::object::UID,
    }

    /// Event emitted whenever a battle result is recorded.
    public struct BattleFinishedEvent has copy, drop {
        avatar_a: address,
        avatar_b: address,
    ...
    entry fun init_admin_cap(ctx: &mut sui::tx_context::TxContext) {
        let id = sui::object::new(ctx);
        let cap = BattleAdminCap { id };
        let sender = sui::tx_context::sender(ctx);
        sui::transfer::transfer(cap, sender);
    }
